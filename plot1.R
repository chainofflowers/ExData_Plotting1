## This is plot1.R, part of the assignment for the CourseProject 1 of the
...
plot1 <- function() {
    ...
    df <- data.frame(c(list(integer(0)),       # integer for as.Date(day)
                       list(character(0)),        # Time (just a character string)
                       rep(list(numeric(0)),7)), # 7 numeric variables
                     stringsAsFactors=FALSE
    )
    colnames(df) <- strsplit(thisLine,';')[[1]]
